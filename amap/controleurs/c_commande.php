	...
	else
	{
		switch($action)
		{
			...
			case 'passerCommande' :
			{
				...
				list($jour, $mois, $annee) = explode('/', $date_livraison);
				if ($annee <= (date('Y') +1) && $annee > (date('Y') -1))
				{
					...
					{
						include('vues/v_erreur_quantite_panier.php');
					}
				}
				else
				{
					include('vues/v_erreur_format_date.php');
				}

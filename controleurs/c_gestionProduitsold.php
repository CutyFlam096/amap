...
switch($action)
{
	...
	case 'ajouterArticle' :
	{
		...
		if($fichier_upload == "img/produits/")
		{//erreut pas d'image
			$insertOK = false;
			include("vues/v_erreur_image_inexistante.php");
		}
		//verifier si un produit du meme nom existe dans la bdd
		else if($pdo->verifProduitExiste($_REQUEST['libelle_produit']) == true)
		{//erreut nom en BDD
			$insertOK = false;
			include("vues/v_erreur_nom_bdd.php");
		}
		else if(in_array($type_Image, $type_autorise) == false)//si le type du fichier est dans $type_aurotise, on l'ajoute et on cree le produit
		{//erreur extension fichier
			$insertOK = false;
			include("vues/v_erreur_type_image.php");
		}
		else if(file_exists($fichier_upload))//si le fichier existe
		{//erreur nom image
			$insertOK = false;
